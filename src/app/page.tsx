  ...
  return (
    <div className="min-h-screen bg-[#0b0f1a] overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#0b0f1a]/95 backdrop-blur border-b border-[#1e2d47]">
        <div className="max-w-md mx-auto flex items-center justify-between px-4 h-11">
          <Link href="/" className="flex items-center gap-1.5">
            <div className="w-7 h-7 bg-gradient-to-br from-[#00e68a] to-[#00bcd4] rounded-md flex items-center justify-center text-[10px] font-bold text-black shrink-0">E</div>
            <span className="text-xs font-bold text-white">EkromNetVPN</span>
          </Link>
          <Link href="/login" className="text-[10px] px-3 py-1 rounded-full bg-[#00e68a] text-black font-bold shrink-0">เข้าสู่ระบบ</Link>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto pt-11">
        ...
        <div className="px-4 pt-5 pb-4 text-center">
          ...
          <h1 className="text-[17px] font-black leading-snug mb-1">VPN ความเร็วสูง <span className="text-[#00e68a]">เสถียร ไม่หลุด</span></h1>
          <p className="text-xs text-[#64748b] mb-3">รองรับ AIS, True, DTAC เริ่ม 50 บาท</p>
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={i} className={`flex items-center gap-3 bg-gradient-to-r ${n.c} bg-[#111927] border ${n.b} rounded-xl p-3`}>
                <div className="w-9 h-9 bg-[#0b0f1a] rounded-lg flex items-center justify-center text-lg shrink-0">{n.i}</div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold truncate">{n.n}</div>
                  <div className="text-[10px] text-[#64748b] truncate">สัญญาณเครือข่าย {n.n}</div>
                </div>
        ...
        {/* Packages - 2x2 grid instead of 4 columns */}
        <div className="px-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {[
              ...
              <div key={i} className={`relative bg-[#111927] rounded-xl p-3.5 text-center border ${pkg.hit ? 'border-[#00e68a]' : 'border-[#1e2d47]'}`}>
                {pkg.hit && <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-[#00e68a] text-black text-[9px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">🔥 ขายดี</div>}
                <div className="text-xl font-bold text-[#00e68a]">{pkg.p}</div>
                <div className="text-[9px] text-[#64748b] mb-1">บาท</div>
                <div className="text-xs font-medium text-white/80 mb-0.5">{pkg.d}</div>
                <div className="text-[8px] text-[#445566] mb-2">2 อุปกรณ์</div>
                <Link href="/order" className="block py-1.5 rounded-lg bg-[#00e68a] text-black font-bold text-[10px]">เลือก</Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="px-4 mb-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[
              ...
              <div key={idx} className="shrink-0 bg-[#111927] border border-[#1e2d47] rounded-xl px-4 py-3 text-center w-[78px]">
                <div className="text-xl mb-1">{a.i}</div>
                <div className="text-[10px] text-[#64748b]">{a.n}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="px-4 mb-4">
          ...
          <div className="space-y-1.5">
            {[
              ...
              "✅ รองรับหลาย Protocol",
              "✅ แอดมินดูแล 24 ชม.",
              "✅ ราคาประหยัด คุณภาพสูง",
            ].map((t, i) => (
              <div key={i} className="bg-[#111927] border border-[#1e2d47] rounded-lg px-3 py-2.5">
